...
function setTokens(interaction, name, amount) {
    ...
    if (name.includes('@')) {
                    ...
                    if (dataG1) {
                        // Found discord
                        db.prepare(`UPDATE users SET tokens = ? WHERE discord_id = ?;`).run(amount, name);
            
                        interaction.reply({content: `${interaction.user} Set ${amount} ERA Coins to <@${name}>.`, ephemeral: false});
        ...
        if (name == 'self') {
            ...
            if (dataG1) {
                // Linked self
                db.prepare(`UPDATE users SET tokens = ? WHERE discord_id = ?;`).run(amount, interaction.user.id);
                

                interaction.reply({content: `${interaction.user} Set ${amount} ERA Coins to your balance.`, ephemeral: false});
            ...
            if (dataG1) {
                ...
                db.prepare(`UPDATE users SET tokens = ? WHERE alderon_name = ?;`).run(amount, name);
