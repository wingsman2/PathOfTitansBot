            ...
            if (dataG2.discord_id !== 'None' && !dataG2.discord_id.includes('P')) {
                ...
                sq.getPlayers(function(err, players){
                    ...
                    if (players) {
                        ...
                        if (amt > dataG2.tokens) {
                            rconCommandStandalone(`whisper ${data.PlayerName} :pot: You do not have ${amt} ERA Coins. Right now you only have ${dataG2.tokens} ERA Coins`, servers[server-1]);
                            return;
                        }
                        if (args.length !== 2) {
                            rconCommandStandalone(`whisper ${data.PlayerName} :pot: Invalid syntax (ex: !withdraw 100)`, servers[server-1]);
                            return;
                        }
                        if (Number.isNaN(amt)) {
                            rconCommandStandalone(`whisper ${data.PlayerName} :pot: Invalid syntax (ex: !withdraw 100)`, servers[server-1]);
